...
namespace AbdelrahmanMedhat\Cashier\Concerns;

use Illuminate\Http\RedirectResponse;
use Illuminate\Support\Collection;
use AbdelrahmanMedhat\Cashier\Cashier;
use AbdelrahmanMedhat\Cashier\CustomerBalanceTransaction;
use AbdelrahmanMedhat\Cashier\Discount;
use AbdelrahmanMedhat\Cashier\Exceptions\CustomerAlreadyCreated;
use AbdelrahmanMedhat\Cashier\Exceptions\InvalidCustomer;
use AbdelrahmanMedhat\Cashier\PromotionCode;
...
trait ManagesCustomer
{
    ...
    {
        return ! is_null($this->cashier_stripe_id);
    }
     ...
     * @return void
     *
     * @throws \AbdelrahmanMedhat\Cashier\Exceptions\InvalidCustomer
     */
     ...
     *
     * @throws \AbdelrahmanMedhat\Cashier\Exceptions\CustomerAlreadyCreated
     */
    public function createAsStripeCustomer(array $options = [])
    {
        ...
        $this->cashier_stripe_id = $customer->id;
     ...
     *
     * @return \AbdelrahmanMedhat\Cashier\Discount|null
     */
     ...
     * @param  string  $code
     * @param  array  $options
     * @return \AbdelrahmanMedhat\Cashier\PromotionCode|null
     */
     ...
     * @return string
     */
    public function billableBalance()
    {
    ...
    public function balanceTransactions($limit = 10, array $options = [])
    {
        ...
        $transactions = static::stripe()
            ->customers
            ->allBalanceTransactions($this->cashier_stripe_id, array_merge(['limit' => $limit], $options));
     ...
     * @param  array  $options
     * @return \AbdelrahmanMedhat\Cashier\CustomerBalanceTransaction
     */
    ...
    public function applyBalance($amount, $description = null, array $options = [])
    {
        ...
        $transaction = static::stripe()
            ->customers
            ->createBalanceTransaction($this->cashier_stripe_id, array_filter(array_merge([
                'amount' => $amount,
    ...
    public function taxIds(array $options = [])
    {
        ...
        return new Collection(
            static::stripe()->customers->allTaxIds($this->cashier_stripe_id, $options)->data
        );
    }
    ...
    public function createTaxId($type, $value)
    {
        ...
        return static::stripe()->customers->createTaxId($this->cashier_stripe_id, [
            'type' => $type,
            'value' => $value,
        ]);
    }
    ...
    public function deleteTaxId($id)
    {
        ...
        try {
            static::stripe()->customers->deleteTaxId($this->cashier_stripe_id, $id);
        } catch (StripeInvalidRequestException $exception) {
            //
        }
    }
